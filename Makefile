...
ORG := github.com/osscontainertools
PROJECT := kaniko
...
install-diffoci:
	@ git clone https://github.com/mzihlmann/diffoci.git && \
		...
		git checkout a51b3ab1ef56a32592e04545b47e7c089ea875d7 && \
		go mod vendor && \
		go install
...
images:
	...
	docker build ${BUILD_ARG} --build-arg=TARGETARCH=$(GOARCH) --build-arg=TARGETOS=linux -t $(REGISTRY)/executor:alpine -f deploy/Dockerfile --target kaniko-alpine .
...
push:
	...
	docker push $(REGISTRY)/executor:alpine
