...
// buildCachedImageWithBuildArg builds dockerfile to kanikoImage with --cache-copy-layers,
// a single --build-arg, and a shared cacheRepo. Used to check whether a build variable
// is reflected in the layer cache key.
func (d *DockerFileBuilder) buildCachedImageWithBuildArg(logf logger, config *integrationTestConfig, cacheRepo, dockerfile, contextDir, kanikoImage, buildArg string) error {
	_, ex, _, _ := runtime.Caller(0)
	cwd := filepath.Dir(ex)
