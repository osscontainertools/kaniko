...
- [kaniko - Build Images In Kubernetes](#kaniko---build-images-in-kubernetes)
  ...
  - [Using kaniko](#using-kaniko)
    ...
    - [Feature Flags](#feature-flags)
      ...
      - [Flag `FF_KANIKO_ASSERT_CACHE_LOOKAHEAD`](#flag-ff_kaniko_assert_cache_lookahead)
...
#### Flag `FF_KANIKO_ASSERT_CACHE_LOOKAHEAD`

Kaniko's build loop now runs in two passes: a precompute pass that derives each stage's final cache key ahead of time, and the build pass that produces layers. Set this flag to `true` to re-run the cache key computation during the build pass and assert that it matches the precomputed value. This is a developer assertion to verify the new precompute pass is correct, there is no benefit to enabling it in production.
Defaults to `false`.
