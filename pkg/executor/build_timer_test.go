...
func TestBuildStopsCommandTimerOnError(t *testing.T) {
	sb := &stageBuilder{
		...
		cmds:  []commands.DockerCommand{failingExecCommand{MockDockerCommand{command: "RUN timer-leak-regression-pin"}}},
	}
	err := sb.build(*NewCompositeCache(""), &config.KanikoOptions{}, util.FileContext{}, &fakeSnapShotter{}, false, nil, nil, &fakeLayerCache{})
	if err == nil {
	...
	// A timer that is never stopped never records its category.
