...
func TestImageNameDigestFile(t *testing.T) {
	...
	opts := config.KanikoOptions{
		NoPush:              true,
		Destinations:        map[string][]string{"default": {"gcr.io/foo/bar:latest", "bob/image"}},
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			opts: config.KanikoOptions{
				NoPush:       true,
				TarPath:      tarPath,
				Destinations: map[string][]string{"default": {"image"}},
			},
			expectedErr: false,
		}, {
...
func TestImageNameTagDigestFile(t *testing.T) {
	...
	opts := config.KanikoOptions{
		NoPush:                 true,
		Destinations:           map[string][]string{"default": {"gcr.io/foo/bar:123", "bob/image"}},
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			opts := config.KanikoOptions{
				...
				Destinations: map[string][]string{"default": test.destinations},
				NoPush:       test.noPush,
