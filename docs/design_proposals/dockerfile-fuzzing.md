...
### Mounts that conflict with base image symlinks

A container runtime can bind-mount a file into the build container, and the base image can ship the directory it lands in as a symlink. kaniko then has to keep the pinned directory and the symlink two names for one directory (mz1073, `FF_KANIKO_PRESERVE_MOUNTED_SYMLINKS`). Eleven defects came out of that mechanism in two days, each found by writing one more fixture, so the fixtures pin what is understood and finding the next one is the fuzzer's job.

The harness mints a base whose `/opt` is a tree of symlinked directories, nested and crossing each other, and a case drawn on that base is built with read-only mounts pinning a random subset of those directories. Which combination is pinned is what a fixture cannot enumerate, and each pinned path is one more swap the build has to get right at once. The generator also draws instructions that reach the tree through both names of a swapped pair: writing through the alias and reading through the target, hardlinking under one, deleting a swapped symlink, and writing through a symlink whose own target is reached through another symlink.

docker gets no mount at all, so the parity oracle compares the swapped result against the mount-free truth, which is exactly the assertion the mz753 and mz1073 fixtures make. The cache, determinism, and invariance oracles mount both sides, so they stay a comparison of like with like. The gated context-shape oracles build unmounted and compare against the mounted fresh image, which asserts the same invariance from the other side: a mount must not change the image.
