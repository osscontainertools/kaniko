...
func AddKanikoOptionsFlags(cmd *cobra.Command, opts *config.KanikoOptions) {
	...
	cmd.Flags().StringVarP(&opts.KanikoDir, "kaniko-dir", "", constants.DefaultKanikoPath, "Path to the kaniko directory, this takes precedence over the KANIKO_DIR environment variable.")
	...
	cmd.Flags().StringSliceVarP(&opts.Target, "target", "", []string{}, "Set the target stages to build")
	...
	cmd.Flags().BoolVarP(&opts.PreCleanup, "pre-cleanup", "", false, "Clean the filesystem before the build")
	...
	cmd.Flags().BoolVarP(&opts.PreserveContext, "preserve-context", "", false, "Preserve build context across build stages by taking a snapshot of the full filesystem before build and restore it after we switch stages. Restores in the end too if passed together with 'cleanup'")
